...
async function getDocumentFromHTMLClipboardItem(
  HTMLItem: ClipboardItem,
): Promise<Document> {
...
async function getCellsFromDocumentBody(
  documentBody: HTMLElement,
): Promise<Compatible<Cell>[][]> {
  ...
  for (let rowIdx = 0; rowIdx < tableRows.length; rowIdx++) {
    ...
    for (let colIdx = 0; colIdx < tableRows[rowIdx].children.length; colIdx++) {
      const rawData =
        tableRows[rowIdx].children[colIdx].getAttribute("data-reactgrid");
...
export default async (): Promise<Compatible<Cell>[][]> => {
  ...
  const document = HTMLItem
    ? await getDocumentFromHTMLClipboardItem(HTMLItem)
    : null;
