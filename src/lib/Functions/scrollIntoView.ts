...
export function getVisibleScrollAreaHeight(
  state: State,
  wholeStickyHeight: number,
): number {
...
export function getCalculatedScrollTopValueToBottom(
  ...
  return (
    scrollTop + location.row.bottom - visibleScrollAreaHeight - topStickyOffset
  );
}
...
export function isFocusLocationOnTopSticky(
  state: State,
  location: Location,
): boolean {
  ...
  return (
    stickyTopRange.rows.length > 0 && row.idx <= stickyTopRange.last.row.idx
  );
}

export function getVisibleScrollAreaWidth(
  state: State,
  wholeStickyWidth: number,
): number {
...
export function isLeftCellAllVisible(
  state: State,
  location: Location,
): boolean {
  const { scrollLeft } = getScrollOfScrollableElement(state.scrollableElement);
...
export function isFocusLocationOnLeftSticky(
  state: State,
  location: Location,
): boolean {
  ...
  return (
    stickyLeftRange.columns.length > 0 &&
    column.idx <= stickyLeftRange.last.column.idx
  );
}
