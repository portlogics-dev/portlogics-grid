...
export class EventHandlers {
  ...
  pointerDownHandler = (event: PointerEvent): void =>
    this.updateState((state) =>
      this.pointerEventsController.handlePointerDown(event, state),
    );
  keyDownHandler = (event: KeyboardEvent): void =>
    this.updateState((state) =>
      state.currentBehavior.handleKeyDown(event, state),
    );
  keyUpHandler = (event: KeyboardEvent): void =>
    this.updateState((state) =>
      state.currentBehavior.handleKeyUp(event, state),
    );
  compositionEndHandler = (event: CompositionEvent): void =>
    this.updateState((state) =>
      state.currentBehavior.handleCompositionEnd(event, state),
    );
  ...
  pasteHandler = (event: ClipboardEvent): void =>
    this.updateState((state) =>
      state.currentBehavior.handlePaste(event, state),
    );
  ...
  pasteCaptureHandler = (event: ClipboardEvent): void => {
    ...
    if (
      htmlData &&
      parsedData.body.firstElementChild?.getAttribute("data-reactgrid") ===
        "reactgrid-content"
    ) {
      event.bubbles = false;
    }
  ...
  protected scrollHandlerInternal = (
  ...
  scrollHandler: () => void = () =>
    this.scrollHandlerInternal(recalcVisibleRange);
  ...
  protected updateOnScrollChange = (
    visibleRangeCalculator: StateModifier,
  ): void => {
    this.updateState((state) => {
